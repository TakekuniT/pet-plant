...
export async function POST(request: NextRequest) {
  try {
    ...
    // For now, send to test email due to Resend testing mode limitations
    const testEmail = 'takekuni@tanemori.org'
    console.log(`Resend is in testing mode - sending to ${testEmail} instead of ${email}`)

    // Use fetch to call Resend API directly with plain HTML
    const emailData = {
    ...
    return NextResponse.json(
      { 
        success: true, 
        messageId: data?.id,
        message: `Invitation email sent successfully! (Test mode: sent to ${testEmail} instead of ${email})` 
      },
      { status: 200 }
    )
